import { FixedPartProba, FixedSet, fixedSetProba, FixedSetProba, TraitProba, WS_URL } from "./constants";
import fs from "fs"
...
export const drawSet = (fixedSetProba:FixedSetProba): FixedSet => {
    return fixedSetProba.map((fixedPartProba:FixedPartProba)=>{
      console.log("fixedPartProba.traits.length",fixedPartProba.traits.length)
        let totalProbaTrait=fixedPartProba.traits.map((probaTrait:TraitProba)=>{return probaTrait.traitProba}).reduce((partialSum, a) => partialSum + a, 0)
...
export const drawSets = (fixedSetProba:FixedSetProba,numberOfSets:number): FixedSet[] => {
  const res = [];
